...
//    @PostMapping(REST_MAPPING_CONSTRAINT.DEFINE_API.UPDATE_PATIENT)
//    public ResponseEntity<BaseApiResponse> updatePatient(
...
//            Long patientId = patientRequest.getPatientId(); // Get the patientId from the request DTO
//            BaseApiResponse response = patientImpl.updatePatient(patientId, patientRequest);
//            if (response.getSuccess() == 1) {
//                return ResponseEntity.status(HttpStatus.OK)
//                        .body(new BaseApiResponse(SUCCESS_OK, SUCCESS, COMMON_MESSAGE_DATA_FETCHED, response));
//            } else {
